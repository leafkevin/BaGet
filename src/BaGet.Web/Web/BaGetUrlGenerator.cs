...
public class BaGetUrlGenerator : IUrlGenerator
{
    ...
    public string GetPackageContentResourceUrl()
        => AbsoluteUrl("v3/package"); 
    public string GetPackageMetadataResourceUrl()
        => AbsoluteUrl("v3/registration");
    ...
    public string GetSearchResourceUrl()
    {
        return _linkGenerator.GetUriByRouteValues(
            _httpContextAccessor.HttpContext ?? throw new ArgumentNullException(nameof(_httpContextAccessor.HttpContext)),
            Routes.SearchRouteName,                values: null);
    }
    ...
    public string GetAutocompleteResourceUrl()
    {
        return _linkGenerator.GetUriByRouteValues(
            _httpContextAccessor.HttpContext ?? throw new ArgumentNullException(nameof(_httpContextAccessor.HttpContext)),
            Routes.AutocompleteRouteName,                values: null);
    }
    ...
    public string GetRegistrationIndexUrl(string id)
    {
        return _linkGenerator.GetUriByRouteValues(
            _httpContextAccessor.HttpContext ?? throw new ArgumentNullException(nameof(_httpContextAccessor.HttpContext)),
            Routes.RegistrationIndexRouteName,                values: new { Id = id.ToLowerInvariant() });
    }
    ...
    public string GetPackageDownloadUrl(string id, NuGetVersion version)
    {
        ...
        return _linkGenerator.GetUriByRouteValues(_httpContextAccessor.HttpContext ?? throw new ArgumentNullException(nameof(_httpContextAccessor.HttpContext)),
            Routes.PackageDownloadRouteName, values: new
            {
                Id = id,
    ...
    public string GetPackageIconDownloadUrl(string id, NuGetVersion version)
    {
        ...
        return _linkGenerator.GetUriByRouteValues(_httpContextAccessor.HttpContext ?? throw new ArgumentNullException(nameof(_httpContextAccessor.HttpContext)),
            Routes.PackageDownloadIconRouteName, values: new
            {
                Id = id,
                Version = versionString
            });
    ...
    private string AbsoluteUrl(string relativePath)
    {
        ...
        return string.Concat(request.Scheme, "://", request.Host.ToUriComponent(), request.PathBase.ToUriComponent(), "/", relativePath);
    }
}
