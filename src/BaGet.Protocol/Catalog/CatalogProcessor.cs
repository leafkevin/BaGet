...
public class CatalogProcessor
{
    ...
    private async Task<bool> ProcessIndexAsync(DateTimeOffset minCommitTimestamp, CancellationToken cancellationToken)
    {
        ...
        var pageItems = index.GetPagesInBounds(minCommitTimestamp, _options.MaxCommitTimestamp);
        _logger.LogInformation("{pages} pages were in the time bounds, out of {totalPages}.", pageItems.Count, index.Items.Count);
    ...
    private async Task<bool> ProcessPageAsync(DateTimeOffset minCommitTimestamp, CatalogPageItem pageItem, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("On page {page}, {leaves} out of {totalLeaves} were in the time bounds.",
            pageItem.CatalogPageUrl, leafItems.Count, page.Items.Count);
        ...
        for (var i = 0; i < leafItems.Count; i++)
        {
            ...
            if (!success)
            {
                _logger.LogWarning("{unprocessedLeaves} out of {leaves} leaves were left incomplete due to a processing failure.",
                    leafItems.Count - i, leafItems.Count);
                break;
            }
        }
    ...
    private async Task<bool> ProcessLeafAsync(CatalogLeafItem leafItem, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else throw new NotSupportedException($"The catalog leaf type '{leafItem.Type}' is not supported.");
        }
        catch (Exception exception)
        {
            _logger.LogError(0, exception, "An exception was thrown while processing leaf {leafUrl}.", leafItem.CatalogLeafUrl);
            success = false;
        }

        if (!success)
        {
            _logger.LogWarning("Failed to process leaf {leafUrl} ({packageId} {packageVersion}, {leafType}).",
                leafItem.CatalogLeafUrl, leafItem.PackageId, leafItem.PackageVersion, leafItem.Type);
        }
