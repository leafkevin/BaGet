...
public class RawCatalogClient : ICatalogClient
{
    ...
    public async Task<CatalogIndex> GetIndexAsync(CancellationToken cancellationToken = default)
        => await _httpClient.GetFromJsonAsync<CatalogIndex>(_catalogUrl, cancellationToken); 
    public async Task<CatalogPage> GetPageAsync(string pageUrl, CancellationToken cancellationToken = default)
        =>await _httpClient.GetFromJsonAsync<CatalogPage>(pageUrl, cancellationToken); 
    public async Task<PackageDeleteCatalogLeaf> GetPackageDeleteLeafAsync(string leafUrl, CancellationToken cancellationToken = default)
        => await GetAndValidateLeafAsync<PackageDeleteCatalogLeaf>("PackageDelete", leafUrl, cancellationToken);
    public async Task<PackageDetailsCatalogLeaf> GetPackageDetailsLeafAsync(string leafUrl, CancellationToken cancellationToken = default)
        => await GetAndValidateLeafAsync<PackageDetailsCatalogLeaf>("PackageDetails", leafUrl, cancellationToken);

    private async Task<TCatalogLeaf> GetAndValidateLeafAsync<TCatalogLeaf>(string leafType, string leafUrl, CancellationToken cancellationToken) where TCatalogLeaf : CatalogLeaf
    {
        var leaf = await _httpClient.GetFromJsonAsync<TCatalogLeaf>(leafUrl, cancellationToken);
        if (leaf.Type.FirstOrDefault() != leafType)
            throw new ArgumentException($"The leaf type found in the document does not match the expected '{leafType}' type.", nameof(leafType));
